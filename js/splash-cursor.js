/**
 * Vanilla port of react-bits SplashCursor (WebGL fluid simulation).
 * @see https://github.com/DavidHDev/react-bits/blob/main/src/content/Animations/SplashCursor/SplashCursor.jsx
 */
const DEFAULTS = {
  SIM_RESOLUTION: 128,
  DYE_RESOLUTION: 1440,
...
function init(container, options = {}) {
  ...
  function getWebGLContext(targetCanvas) {
    ...
    if (!isWebGL2) {
      context = targetCanvas.getContext('webgl', params)
  ...
  function resizeCanvas() {
    const width = scaleByPixelRatio(canvas.clientWidth || container.clientWidth);
    const height = scaleByPixelRatio(canvas.clientHeight || container.clientHeight);
  ...
  function getPointerPosition(e) {
    ...
    return {
      posX: ((e.clientX - rect.left) / rect.width) * canvas.width,
      posY: ((e.clientY - rect.top) / rect.height) * canvas.height,
    };
  }
  ...
  function handleMouseDown(e) {
    ...
    updatePointerDownData(pointer, -1, coords.posX, coords.posY);
    clickSplat(pointer);
  }
  ...
  function handleMouseMove(e) {
    ...
    if (!firstMouseMoveHandled) {
      const color = generateColor();
      updatePointerMoveData(pointer, coords.posX, coords.posY, color);
      firstMouseMoveHandled = true;
    } else {
      updatePointerMoveData(pointer, coords.posX, coords.posY, pointer.color);
    }
  }
